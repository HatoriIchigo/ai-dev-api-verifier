...
7.1. **データソース到達の必須化（呼び出しレベル・ルール3/7の強化）**: ルール3（layer1 は repository を
   import）・ルール7（layerN は下位レイヤーを import）は **import の有無**しか見ないため、「import だけ書いて
   実際には呼ばず、repository／下位レイヤーに降りる前に `return` してしまう」抜け道が残る（入力のエコーや
   定数からの組み立てで DTO をでっち上げる握りつぶし）。これを塞ぐため、**戻り値型が `dto/**` の public メソッド**の
   うち、クラス内の他メソッドから呼ばれていないもの（＝上位から呼ばれるエントリ）は、本体内または**同一クラス内の
   委譲チェーン経由**でデータソースのメソッド呼び出しに到達すること。到達しなければエラー。
   - データソースは layer1=`repository`、layerN（N≥2）=自分より下位の `layer`。
   - 内部ヘルパ／純変換メソッド（クラス内から呼ばれているもの）は対象外（誤検知抑制）。private・void・
     プリミティブ戻りも対象外。
   - **既知の限界**: クラスをまたぐ完全な制御フロー解析は行わない。分岐単位で「ある return パスだけ
     データソースを経由しない」ケースは検出しきれず、ここは結合テスト（ルール17/18）が実連携を通すことで
     担保する。AST 検査のため `java-builder` のみで判定する。
...
> **依存集合の定義（ルール12・13 共通）**: 本節での「依存集合」は、対象コンポーネントの
> **直接 import**（`imports` に列挙された宛先文字列の集合。重複は除く）を指す。推移閉包ではなく直接 import で
> 比較するため、設計 JSON から決定的に判定でき、誤検知が出にくい（「重複する直接 import を禁止」という原則と整合）。

11. **飛び越し参照の禁止（※ルール14に内包）**: `C → B`（`B` が下位レイヤー）で、間のレイヤーのクラス `A` が
    既に `B` を import している場合、`C` は `B` を直接 import できない（`A` を経由する）という規則。
    ただし**間のレイヤーが存在する時点でレイヤー差は必ず ≥2 となり、より厳格なルール14（差≥2を一律禁止）で
    既にエラー**になる。strict layering を採るため本ルールは**ルール14に内包され、単独では発火しない**
    （`layered-checker` でも別途実装しない）。relaxed layering へ方針変更する場合にのみ独立した意味を持つ。
12. **同一レイヤーの依存包含＝昇格**: 同一レイヤーの相異なる2コンポーネント `a` / `b` の**直接 import 集合**を
    `A` / `B` とするとき、次を**いずれもエラー**とする。【構造判定可】
    - **真包含** `B ⊊ A`（`B` が非空で `A` が `B` を真に含む）: `a` は `b` の依存を丸ごと内包しており、同一レイヤーに
      置くと依存が重複する。**`a` を1段上位へ昇格し、`a` は `b` を import して共通依存を `b` 経由で使う**。
    - **集合一致** `A = B`（非空・`a ≠ b`）: 2コンポーネントの直接依存が完全に同じ。責務が重複している兆候であり、
      **統合するか責務を分離**する（報告は名前昇順で1回）。
13. **同一レイヤーの依存重複（部分共有）→ 切り出し＋昇格**: 同一レイヤーの2クラスが直接 import 集合を**部分的に**
    共有する（互いに包含せず共通部分が非空）場合、共通依存を制御する新クラスへ切り出し、両者を昇格して共通依存の
    直接 import を禁止する。**ただし「どれだけ重なれば切り出すべきか」はしきい値が設計判断であり一意でないため、
    現時点では `layered-checker`／`java-builder` のいずれも未実装**（将来、共有件数しきい値を決めて対応する）。
14. **レイヤー差の制限＝降格**: `X → Y`（`Y` が下位）で `layer(X) - layer(Y) >= 2` の場合、`X` を
    `layer(Y)+1` へ**降格**する（基準は下位レイヤー）。＝隣接レイヤーへの依存のみ許す strict layering。【構造判定可】

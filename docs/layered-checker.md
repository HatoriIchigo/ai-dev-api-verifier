...
- 命名サフィックス（`repository`=`*Repository`／`dto.in`=`*InDto`／`dto.out`=`*OutDto`） — [code-rule D5]
- `repository/<Stem>Repository` ↔ `dto/in/<Stem>InDto`・`dto/out/<Stem>OutDto` の対応（各ちょうど1件） — [code-rule 4]
